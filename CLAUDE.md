...
uv sync --extra mujoco  # MuJoCo backend for module 7b (robot_mujoco.py)
uv sync --extra warp  # MuJoCo Warp batched backend (MujocoWarpArm; CUDA GPU or Warp CPU fallback)
...
- `thermal.py` — `RosenthalThermal` (analytic moving point source) **and** `GoldakFDM` (3D transient double-ellipsoid FDM). The two are cross-validation of each other; `GoldakFDM` uses a half-symmetry domain with edge-pad Neumann boundaries and per-step heat-source renormalization for exact energy conservation. `GoldakFDM` is typically driven by `GMAWDynamics`' steady power. Passing `weave=` (a `weave.py` object) gives the heat source a lateral offset `y_s(t)`; since that breaks the y=0 mirror symmetry, the constructor then switches to a **full-width grid** (`Ny = 2*Ly/dx - 1`, y=0 on a cell center, `j_center` marks the centerline, ~2× cost). Beware: the half model's edge-pad mirror sits at the face `y=-dx/2`, the full grid mirrors through the y=0 cell center — so pointwise T near the axis differs O(dx) between the two even at zero weave amplitude (pool L/W/D and total energy match; don't chase bit-identity). Passing `convection=` (an `EffectiveMarangoniCorrection`) multiplies in-pool conductivity by `alpha_eff()/alpha` (limiter-capped, mushy-zone ramp) and switches the diffusion term to a variable-coefficient flux form (harmonic-mean face k, energy-conserving; reduces exactly to the 6-point Laplacian at κ=1); `dt` shrinks by the same factor (~6× steps, ~12× wall). `convection=None` keeps the constant-k path bit-identical to the pinned README results. Note: hydra `instantiate(cfg.goldak, convection=obj)` kwarg-overrides re-structure the dataclass into a mapping — `GoldakFDM.__init__` reconstructs it; prefer composing with `convection=effective` instead. See `docs/melt_convection_assessment.md` for the option analysis (effective-k vs kinematic vs full NS vs OpenFOAM).
- `weave.py` — torch-weaving motion models consumed by `GoldakFDM(weave=...)`: `HarmonicWeave` (sine/triangle, `amplitude_m` is **peak-to-peak**) and `WaypointWeave` (摆动库 `weave_pattern` waypoint waveforms, normalized so max |y_pct| maps to A/2; vertical `z_pct` deliberately not modeled). `RobotExecutedWeave` wraps a sampled TCP trajectory from `SixDofArm.track_path` as a weave object (`from_tracking(t, tip, p0, v)`; seam assumed along robot base x at constant v; `amplitude_m` is the *executed* peak-to-peak) — this is how robot-executed weaving is injected into `GoldakFDM` (see `notebooks/robot_deployment_scenarios.ipynb`). Weave frequency (DB range 0.3–4 Hz) is ~2 orders below the droplet Rayleigh mode (~527 Hz), so weaving moves the heat source but does not couple into module 6.
...
- `robot_vi.py` — `TwoLinkArm`: two-link welding-robot arm, config-dependent `M(q)`; `SixDofArm`: spatial 6R extension (standard DH; spherical wrist by default, or pass full `dh_d/dh_a/dh_alpha_deg` tables for other geometries — `model/robot6_ur5e.yaml` parameterizes it as a UR5e with UR's published DH + link masses, offset wrist; everything downstream is numeric so no spherical-wrist decoupling is assumed). Its `M(q)` is assembled numerically per step from COM Jacobians (cylinder links + reflected rotor inertia `J_rotor`, which keeps `M(q)` nonsingular for the wrist-roll axes and well-conditioned) — there is no analytic dynamics, so its `lagrangian` must stay cheap; `track_path(p_ref_fun, ...)` tracks an arbitrary 3D reference (seam_tracking is its straight-line special case); the implicit DEL makes module 7b the slowest part of `welding-sim-vi` (~1 min at defaults, trim via `run.robot6.passive_t_end`).
- `robot_mujoco.py` — `build_mjcf(arm)` generates an MJCF model programmatically from a `SixDofArm` (single parameter source: DH table, cylinder links, `J_rotor` → joint `armature`; contact disabled); `MujocoArm` wraps MjModel/MjData for cross-validation (`mass_matrix`/`fk_tip` agree with `_MV`/FK to ~1e-15 — re-check this after touching either side), `passive_rollout` (Euler/implicitfast/RK4), and `track_path` sharing the *same* `_pd_tracking_law` closure as the VI backend, plus MuJoCo-only joint damping/frictionloss. `MujocoWarpArm` is the batched alternative backend (optional `mujoco-warp`, extra `warp`): same MJCF, nworld worlds stepped in lockstep by NVIDIA Warp kernels — thousands of parallel worlds on CUDA, automatic Warp-CPU fallback otherwise (works on this repo's macOS dev machines; validated there). It computes in float32 (fine for sweeps/statistics, not for long-horizon energy claims) and its physics timestep is fixed at construction. Torque laws are evaluated per-world on the CPU (`track_batch(tau_funs, ...)`), which is the device sync point. Per-world joint losses (domain randomization) are supported: model loss fields are expanded at construction via `put_model(batch_sizes=...)` and set with `set_joint_losses(damping, frictionloss)`. mujoco/warp are optional dependencies lazy-imported in the classes' `__init__`; the module top level only needs numpy. See `notebooks/robot_mujoco_validation.ipynb`. Do not present MuJoCo as a replacement for the VI modules — at the VI demos' large steps (h=30 ms) MuJoCo's Euler diverges; the engines are complementary (throughput + actuator models vs long-horizon energy fidelity).
