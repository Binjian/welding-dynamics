...
### 模块 7b — 六自由度机械臂扩展 (`robot_vi.py` 的 `SixDofArm`)

空间 6R 球腕机械臂（标准 DH），连杆按实心圆柱建模并计入折算到
关节侧的电机/减速器惯量 J_rotor（真实焊接机器人腕轴惯量的主项，
同时保证 M(q) 良态）。M(q) 由各连杆质心 Jacobian **逐步数值装配**，
L = ½q̇ᵀM(q)q̇ − V(q) 直接喂给同一 `MidpointDEL` —— 高自由度下
无需解析动力学正是变分积分器的实用优势。60 s 无驱动摆动 (h=30 ms)：
VI 能量误差有界振荡（~1% 量级），RK4 同步长单调漂移至 ~15%。
三维焊缝跟踪（位姿 DLS 逆解 + 按 M(q) 对角元整定的逐关节 PD +
重力补偿）RMS 误差 0.42 mm（`m7b_robot6_vi.png`）。默认参数下
本段约需 1 分钟，可用 `run.robot6.passive_t_end=20` 缩短。

几何可经 `dh_d/dh_a/dh_alpha_deg` 换成任意标准 DH 表 —
`model@robot6=robot6_ur5e` 即以 Universal Robots UR5e（reach 850 mm，
官方 DH 与连杆质量，偏置腕)参数化，见
`notebooks/robot6_weave_interactive_demo.ipynb`。
`track_path(p_ref_fun, t_end)` 可跟踪任意三维参考轨迹（焊缝跟踪是其
直线特例）；配合 `weave.RobotExecutedWeave`（把跟踪仿真的实际 TCP
轨迹包成 `GoldakFDM` 可用的摆动对象）可将"机器人执行的摆动"注入
热场仿真，对比理想摆动与机器人执行的熔池差异、评估工艺预设的动态
可执行性 —— 完整部署场景分析见
`notebooks/robot_deployment_scenarios.ipynb`。

**MuJoCo 后端** (`robot_mujoco.py`，可选依赖 `uv sync --extra mujoco`)：
`build_mjcf(arm)` 由同一份 DH/惯量参数程序化生成 MJCF（`J_rotor` →
joint `armature`），`MujocoArm` 与变分后端共享同一 PD 力矩律 ——
M(q)/FK 交叉核对到 ~1e-15。互补分工：变分积分器买大步长下的长期
能量保真（h=30 ms 时 MuJoCo 半隐式 Euler 发散、MidpointDEL 有界）；
MuJoCo 买小步长吞吐（同问题 1 ms × 60 k 步约 4 s）与执行器模型 ——
关节阻尼+库仑摩擦使摆幅 −31%、熔宽 −14%，理想模型看不到这条链路。
批量替代后端 `MujocoWarpArm`（`uv sync --extra warp`）用 NVIDIA Warp
内核同步积分 nworld 个世界：CUDA GPU 可上千世界并行做参数扫描，无
GPU 自动落到 Warp CPU 设备 —— 5 频点摆频扫描一次 `track_batch` 完成，
幅值比曲线与 VI 串行扫描逐点吻合（float32，短程轨迹差 ~1e-6）。
见 `notebooks/robot_mujoco_validation.ipynb`。
